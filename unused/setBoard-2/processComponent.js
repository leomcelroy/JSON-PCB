...
import { shapesToPathData } from "../views/dom/renderShapesToSVG.js";
...
export function processComponent(comp, board) {
  ...
  footprint.pads.forEach((pad) => {
    ...
    pad?.traces?.forEach((trace) => {
      ...
      const layers = newTrace.layers;

      traces.push({
        ...newTrace,
        layers: layers.map((layer) => (flip ? swapFB(layer) : layer)),
      });
    });
